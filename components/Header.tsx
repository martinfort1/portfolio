...
export default function Header() {
  ...
  return (
    <header className='z-[999] relative'>
        ...
        <nav className='flex fixed top-0 left-1/2 h-[4.5rem]
        -translate-x-1/2 sm:top-6 sm:h-[3.25rem]'>
                ...
                {links.map( link => (
                    <motion.li className='h-3/4 flex items-center justify-center' key={link.hash}
                        ...
                        <Link className={clsx("flex w-full items-center justify-center px-3 py-3 hover:text-gray-950 transition dark:text-gray-500 dark:hover:text-gray-300", {
                            "text-gray-950 bg-gray-200 rounded-full dark:!text-gray-200 dark:bg-gray-800": activeSection === link.name,
                        })}
                        href={link.hash}
                        onClick={() => {
