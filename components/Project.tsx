...
export default function Project({ title, description, tags, imageUrl, siteUrl }: ProjectProps) {
  ...
  return (
    <motion.div
        ...
        >
        <section
          className="bg-gray-100 max-w-[42rem] border border-black/5 rounded-lg overflow-hidden hover:bg-gray-200 transition relative flex flex-col sm:h-[20rem] group-even:pl-8 dark:bg-white/10 dark:hover:bg-white/20"
        >
          <a
            href={siteUrl}
            target="_blank"
          ...
          >
            <div className="pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 relative z-10 sm:max-w-[50%] flex flex-col h-full sm:group-even:ml-[18rem]">
              ...
              <ul className="flex flex-wrap mt-4 gap-2 sm:mt-auto">
                {tags.map((tag, index) => (
                  <li
                    className="bg-black/[0.7] px-3 py-1 text-[0.7rem] uppercase tracking-wider text-white rounded-full dark:text-gray-300"
                    key={index}
                  >
                    {tag}
                  </li>
